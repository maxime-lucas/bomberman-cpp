...
void Game::evolue(Input& in)
{
	...
	for(int i=0; i<NB_PLAYERS; i++ )
	{
		...
		if( in.getKey(tabkey[i][4] ) )
		{
			...
			if( p->getNbBomb() > 0 )
			{
				...
				int y = p->getDimCoordEcranY();
				
				b->setDimCoordEcranX(x);
				b->setDimCoordEcranY( y + (p->getDimCoordSpriteH() - b->getDimCoordSpriteH() ) );
	...
	{		
		...
		{			
			...
			int y = b->getDimCoordEcranY() + TILE_HEIGHT;
			x = x - x % TILE_WIDTH;
			y = y - y % TILE_HEIGHT;
	...
	{		
		...
		if( SDL_GetTicks() >= e->getDateOfDisparition() )
		{
